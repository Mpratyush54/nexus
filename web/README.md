...
A running Central Server (`internal/server/`, Phase 1.8 REST). The WebSocket hub
(`internal/server/ws.go`, plan §3.2) does **not** exist yet — the dashboard
implements the planned protocol now, so it lights up the moment the hub lands.
Without WS it remains usable over REST (with 5s polling fallback).
...
The API server is expected at `http://localhost:8080` and WS at
`ws://localhost:8080/ws` — both editable in §1 of the UI and persisted to
`localStorage`.

## Use

1. §1: set API base + WS URL → **Login** (`POST /auth/login`, token stored).
2. §2: enter `folder_name` (or canonical URL) → **Resolve project**, or paste a
   known `project_id`. Enter a `session_id` (opaque until the sessions migration
   lands) → **Subscribe** sends `{"type":"subscribe","project_id":…}`.
...
4. §5: memory list via `GET /memory/search`; **Confirm / Reject / Promote**
   buttons try `POST /memory/{id}/confirm|reject|promote` (plan §2.8 endpoints,
   not on the server yet) and always apply optimistic local state + a WS
   `action` hint for the future hub.
...
- Real-time updates without full-page refresh → §4 feed + §§5–6 in-place
  card updates; `presence`/`memory_update`/`episode_update` handlers mutate
  single DOM nodes only.
- Confirm proposed memories from the UI → §5 buttons (optimistic now,
  server endpoints pending — see `docs/decisions/2026-09-17-dashboard.md`).
