...
func BuildPrompt(project string, existing []Existing, turns []Turn) string {
	...
	sb.WriteString("- Phrases like \"we decided\", \"we'll use\", \"we use X for Y\" ARE durable — include them.\n")
	sb.WriteString("- Only return an empty list when there is truly nothing reusable.\n\n")
